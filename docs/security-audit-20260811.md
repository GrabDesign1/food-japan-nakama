...
## 4-3. 残課題の実施（2026-08-11 第2弾）

migration=`member_withdrawal`（`Member.withdrawalRequestedAt` / `withdrawalReason` を追加）。
Supabase に**非公開バケット `message-attachments`** を新設（既存の添付は0件だったため移行不要）。

| 指摘 | 対応 |
|---|---|
| 高-2 添付が公開バケット | 添付を非公開バケットへ。DBには保存パスのみを持ち、配信は `/api/attachments/[messageId]` が「スレッド参加者か」「引き合い課金でマスクされていないか」を確認してから**60秒の署名付きURL**へリダイレクト。公開URLでの直接取得は不可 |
| 中-10 現行パスワード未確認 | 再設定リンクを踏んだ直後だけ `nakama-pw-recovery` クッキー（httpOnly・15分）を発行し、それが無いセッションでは**現在のパスワードで再認証**してから変更。変更完了時に本人へ通知メール |
| 中-9 REVIEWER の権限 | 優良案件マーク／解除（紹介料1,100→3,300円）・条件一致通知の送信／否認・掲載オプションの否認を `requireSuperAdmin` に引き上げ。UI側も出し分け |
| 応募者限定公開の秘匿漏れ | 社名だけでなく**都道府県・市区町村・業種**も非開示に（案件に書かれた地域のみ表示） |
| 中-13 退会・孤児ファイル | `/profile` に退会の申し出（理由任意・確認チェック必須）。即時削除はせず申請として記録し、事務局へ通知＋監査ログ。`/admin/members` に申請一覧（Premium課金中は警告表示）。`deleteMember` は会員画像・ロゴ・アバター・一時アップロード・案件/PJ画像・**メッセージ添付**を Storage から削除し、削除件数を監査ログに記録 |
| 低 条件一致通知 | `visibility === "public"` の案件のみ送信（開けないリンクを送らない）。送信上限を注文明細の商品から取得（ハードコード撤去） |
| 低 監査ログの不足 | お知らせ・記事・バナー・相談ステータスの作成/削除/切替に `writeAudit` を追加 |
| 低 テナント条件 | favorites の台帳お気に入りにテナント条件＋`visibility` 判定を追加 |
| 低 単品クレジットの期限 | 単品購入（1,100/3,300円）は**無期限**に変更（購入画面で期限を告知していないため）。パックのみ180日 |
| 低 未読返還の失効 | 返還先の元ロットが期限切れなら、新しいロットとして期限を切り直して発行 |
| 低 P2002 の握りつぶし | `$transaction` 内の `create`＋catch を `createMany({ skipDuplicates: true })` に置換（Postgres ではトランザクション内で例外を握ると以後のクエリが必ず失敗するため） |
| 低 Checkout 連打 | 直近2分以内の同一条件の未決済注文があれば、そのCheckoutセッションを開き直す（注文とセッションの無制限生成を防止） |

### 検証結果（すべて実測）

- `npx tsc --noEmit` / `vitest` 18件 / `next build` すべて成功（ESLintの既存エラー2件は変更前から）
- **添付の配信制御**：公開URLでの直接取得は 400、参加者＋無料メッセージは 307→署名付きURLで内容取得可、
  マスク対象メッセージの添付は 403、未ログインはログインへ、送信者本人（買い手）は取得可
- **パスワード変更**：通常セッションで `/reset-password` を開くと「現在のパスワード」欄が出る
- **退会**：`/profile` に退会の申し出セクションが表示される
- **応募者限定公開**：無関係の第三者から見て、社名・都道府県・市区町村・業種がいずれもHTMLに出力されない
  （やり取りのある相手には従来どおり開示される）
- 検証用アカウント・案件・スレッド・添付ファイルはすべて削除済み
