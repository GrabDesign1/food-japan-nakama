...
- **トップの置き換えと実績ページ（2026-08-17・本番反映済み。commit b27b170・指示書＝相談ファイル/CloudCode_トップページ改善指示書.md ＋ nakama-top-ui-preview.html）**: 指示書のうち**事業モデルに関わる部分は不採用**（下記）。UIは「追加」ではなく**既存セクションの置き換え**で実施し、セクション数は10のまま。**DB変更なし・migrationなし**。
  ...
  - **⚠️掲載0件のうちは案件セクションを出さない（ユーザー決定 2026-08-17）**＝`MIN_LISTINGS_TO_SHOW = 4`（`src/lib/public-content.ts`）。**売りたい／探している／共創PJで別々に判定**し、満たないセクションは `PreviewSection` が `null` を返して**丸ごと消える**。「現在ありません」が3つ並ぶと誰もいないサービスに見えるため。4件にしたのは `lg:grid-cols-4` で1行が埋まる数だから。**閾値を変えるならこの定数1つ**。
...
- **Food Japan Summit の協賛申込フォーム `/sponsor`（2026-08-17・本番反映済み）**: サミットの協賛企業を集めるための**独立したフォーム**。**⚠️NAKAMA本体の機能ではない**＝ヘッダー・フッターのナビからリンクせず、`sitemap.ts` と `llms.txt` にも入れず、`robots: noindex` にしてある（**URLを直接案内して使う**というユーザー指示 2026-08-17）。公開したくなったら noindex を外して sitemap に足すだけ。
  ...
  - **切り替えの仕様（2026-08-17 に名古屋・両開催を追加して作り直し）**＝設問1のラジオで**5コース**（宮崎のみ／名古屋のみ／両開催／宮崎県法人特別割／相談）を選ぶと、**日程・会場・プラン一覧・設問2の価格がまるごと入れ替わる**。価格＝宮崎/名古屋 15/30/50/80/250万、両開催 20/50/80/120/400万、特別割 15/30/40/70/200万（すべて税別）。**サーバー側でも「選んだコースに存在するプランか」を検証**する（送信値の付け替え防止）。年間会員は協賛と併用できるのでコース選択とは独立したチェックボックスにした。
  - **⚠️ラジオ/チェックボックスには `autoComplete="off"` が必須**。これが無いとブラウザが再読み込み時に選択を復元し、**Reactの state と表示がずれる**（チェックボックス版で実際に踏んだ）。
  - **⚠️募集資料PDF（`~/Desktop/00_デスクトップ/企画書/スナックフォーラム/2026_JapanFoodSummit/2026_宮崎_協賛募集資料_JapanFoodSummit.pdf`）に矛盾があり、フォームには未反映のものがある**（2026-08-17 にユーザーへ確認依頼中）: ①**宮崎県法人特別割のカンファレンスパス人数が通常プランと逆転**（p.11 STANDARD 30万でパス1名／通常p.9は同額で2名。PRESENTER 40万で3名／通常50万で2名）→ **確定するまで特別割は価格だけ出し、特典の内訳は出していない** ②p.10の説明文「11月3日・4日」はp.9の「11月17日18日」と矛盾（資料側の誤植）③p.9は「愛知のみ」、他は「名古屋」で表記ゆれ ④LIGHTの「名簿掲載」が NAKAMA掲載を指すのか不明。
  - **PDFで裏が取れたのでユーザー文面の抜けを補った箇所**＝PRESENTER/STRATEGIC の「協賛ロゴ掲載」「コワーキングルーム使用」、PRESENTER以上の「参加後の参加者リストのご提供」。`sponsor.ts` のコメントに「※」で明示してある。
