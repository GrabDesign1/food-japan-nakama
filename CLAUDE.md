...
- **会員一覧を名刺台帳型にして、選択→DM配信／CSV書き出し（2026-08-16・ユーザー指示。Eightの一覧を参考）**: `/admin/members` を作り替えた。
  ...
  - 検証＝全選択で5社選択・広告に切り替えると2社に絞られる・CSVのヘッダーと1行目・一括送信1社の実送信と対応履歴／監査ログを実機で確認。**テスト記録と一時アカウントは削除済み**。

## やることリスト（対外募集開始前）
0000. **【追加】2026-08-16 時点で残っている宿題（この4つが直近）**:
