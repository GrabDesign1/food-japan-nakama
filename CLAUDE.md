...
**NAKAMA本体の機能ではない**＝ヘッダー・フッターからリンクしない。**案内ページ `/sponsor` は 2026-08-19 に検索対象へ変更**（`sitemap.ts`／`llms.txt` にも掲載。下の SEO/AIO の項を参照）。**申込・相談フォームは `robots: noindex` のまま**＝URLを直接案内して使う。定義は **`src/lib/sponsor.ts` に集約**（949行・サーバー依存を持たせない＝クライアントからも import する）。
...
- **Food Japan Summit の協賛ページ `/sponsor`（2026-08-17・本番反映済み）**: サミットの協賛企業を集めるための**独立したページ群**。**3ページ構成**＝`/sponsor`（案内＋2つの入口）／`/sponsor/apply`（プラン選択を含む本申込・設問18）／`/sponsor/contact`（**連絡先だけの短い相談フォーム**）。**⚠️入口を2つに分けたのは「金額・プランを決めてからでないと押せない状態」をなくすため**（ユーザー指示 2026-08-17）。案内ページの本文は確定コピー「協賛ではなく、共創へ。」。**相談フォームの必須は組織名・担当者名・電話番号・メールアドレスの4つだけ＋任意でFacebook URLとご相談の内容。ここに項目を足さないこと**（足すと「まず相談したい人」が離脱する）。受付番号は申込が `FJS-`、相談が `FJS-Q-` で区別できる。
  ...
  - **⚠️SEO/AIO（2026-08-19 ユーザー指示「/sponsor のみ単独の SEO と AIO」）**＝**案内ページ `/sponsor` だけ検索・AI検索の対象にした**。①page.tsx に `robots` を書かず既定（index, follow）に戻す ②タイトルを「Food Japan Summit 2026 協賛募集｜宮崎11/17-18・名古屋12/15-16」に、description に登壇枠・価格の下限・特別割・ブースを入れた ③**Event の構造化データを2件**（宮崎・名古屋）埋め込み ④`sitemap.ts` に追加＋`CONTENT_UPDATED_AT` 更新 ⑤`llms.txt` に「Food Japan Summit 2026 の協賛」節（全プラン価格・特典・締切）と主要ページの1行を追加。⚠️**`/sponsor/apply` と `/sponsor/contact` は noindex のまま**（フォームなので検索に出さない。sitemap にも足さない）。⚠️**Event の `offers` は入れない**＝schema.org では来場チケットの販売を指すので、協賛プランを入れると入場券と誤解される（価格はllms.txt側に文章で置く）。⚠️構造化データの日付は**日付だけ**（資料が「時間調整中」のため）。⚠️**2026-08-19：公開ページ共通フッターの著作権行に、`/sponsor` への小さいテキストリンクを1本追加した**（ユーザー指示）＝文言は「Food Japan Summit 2026 協賛募集・協賛スポンサー募集」（11px・muted）。**サイト内から `/sponsor` へ張る唯一のリンク**で、Search Console に「URL が Google に認識されていません」と出たための発見経路。**外すとサイトマップだけに戻る**。ヘッダーとフッターのナビ5列には入れていない（NAKAMA本体の機能ではないため）。閉じたいときは page.tsx に `robots: { index: false, follow: false }` を戻し、sitemap と llms.txt からも外すこと。
