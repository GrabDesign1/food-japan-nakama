...
- **Food Japan Summit の協賛申込フォーム `/sponsor`（2026-08-17・本番反映済み）**: サミットの協賛企業を集めるための**独立したフォーム**。**⚠️NAKAMA本体の機能ではない**＝ヘッダー・フッターのナビからリンクせず、`sitemap.ts` と `llms.txt` にも入れず、`robots: noindex` にしてある（**URLを直接案内して使う**というユーザー指示 2026-08-17）。公開したくなったら noindex を外して sitemap に足すだけ。
  ...
  - **切り替えの仕様**＝冒頭のチェックボックス「宮崎県内に本店または主たる事業所を置く法人」を入れた瞬間に、**プラン一覧・設問1の申込区分・設問2の価格がすべて特別割だけに入れ替わる**（PRESENTER 50→40万／STRATEGIC 80→70万／DIAMOND 250→200万。LIGHT・STANDARDは同額）。**サーバー側でも整合を検証**（チェックなしで特別割区分、チェックありで通常区分は弾く）。
  - **⚠️チェックボックスには `autoComplete="off"` が必須**。これが無いとブラウザが再読み込み時にチェック状態を復元し、**Reactの state は false のまま表示だけ特別割になる**（実際に踏んだ）。
