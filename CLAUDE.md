...
- **初回紹介料（中心商品）**: 売り手→「探している」案件への最初の提案のみ有料。通常1,100円／NAKAMA確認済み優良案件3,300円／5件パック5,500円／10件パック11,000円（税込・パック期限180日）。**パックは1件あたり1,100円＝単品と同額**（まとめ買い割引は2026-08-11の価格整合で廃止。会費より安く買えて会員プランが破綻するため）。**パックは会員割引の対象外**。事業者確認（承認）時に組織単位で3件付与（一度だけ）。送信後14日未読なら1件自動返還（開封済み・返信なしは返還しない）。課金単位=売り手×案件（ContactUnlock unique）。
- **月額22,000円（税込）＝NAKAMAビジネス会員**（2026-08-11確定・旧「Premium会員」）: 特典＝**毎月30件の提案チケット（繰越なし・1件あたり733円）＋追加チケット（1件購入）と掲載オプションが20%割引**。提案無制限は「大量営業・スパムで買い手が離れる」懸念から撤回。**価格整合の要点（2026-08-11 見直し）**＝会員が得になる分岐点は「月額 ÷ 1件あたりの購入単価」で決まり、付与数は上限にしか効かない。旧設定（月20件・パック770円/件）は分岐点28.6件＞上限20件で**フル消化しても損**だった。パック単価を1,100円に統一＋付与30件で分岐点20件＜上限30件となり「月20件以上提案するなら会員が最安」が成立。既存契約1件=グラブデザイン（9/9満了予約）。3か月後に使用率・返信率・商談化率を見て再調整する。
