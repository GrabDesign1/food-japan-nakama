...
- **リードの初回開封に1クレジット（2026-08-12・ユーザー決定。⚠️ブランチ `lead-unlock` で作業中・未マージ／migration `lead_unlocks` のみ本番適用済み）**: 「探している」への提案だけが有料で、「売りたい」で得たリードは無料だった＝**収益が片側に偏り、売り手側の価値が回収できていない**。**「初回の接点に紹介料」という一本のルール**に揃える。
  ...
  - **⚠️マージ前に必ず要るもの（未了）**：①**提案一覧（/ledger/[id]/proposals）の伏せ字**＝いまは本文抜粋が全部見えており、**払わずに読めてしまう**。②**規約・/pricing・/faq・/billing・llms.txt・トップの改定**＝「届いた問い合わせへの返信は何往復でも無料」が全ページに残っている。**無料範囲を狭める不利益変更**なので施行日と周知が要り、弁護士への事後確認は7回目になる。③未開封7日通知（日次バッチ）④返信率の表示⑤問い合わせフォームの予告。
