...
export const metadata = {
  ...
  description:
    "登録・掲載・応募・問い合わせは無料。事務局による共創テーマ設計、パートナー探索、商談、実証・事業化支援は個別契約です。",
};
...
const LISTING_OPTIONS: { name: string; price: string }[] = [
  ...
  { name: "条件一致通知（最大100件）", price: "11,000円" },
    ...
    >
      ...
      <section className="rounded-[12px] border border-[var(--green)] bg-white p-6">
        ...
        <ul className="mt-3 flex flex-col gap-1.5 text-[13px] leading-6 text-[var(--ink-2)]">
          <li>・プロフィール登録</li>
          <li>・案件（販売・提供できる商品／仕入れ・調達したいもの／共創パートナー募集）の閲覧・登録</li>
          <li>・応募・問い合わせ</li>
        ...
        <p className="mt-3 text-[12px] leading-6 text-[var(--muted)]">
          ...
          売り手から「仕入れ・調達したいもの」案件への初回提案のみ、紹介料（通常案件1,100円・確認済み優良案件3,300円・パック割引あり）がかかります。同じ案件・同じ相手との継続メッセージと、受けた問い合わせへの返信は何往復でも無料です。
        </p>
        <div className="mt-4">
      ...
      <section className="rounded-[12px] border-2 border-[#C9A053] bg-[#FDF9EF] p-6">
        ...
        <ul className="mt-3 flex flex-col gap-1.5 text-[13px] leading-6 text-[var(--ink-2)]">
          <li>・<b>毎月20件の提案チケット</b>（「仕入れ・調達したいもの」案件への初回提案に使えます。翌月への繰越はありません）</li>
