...
import { setProjectBodyImage, clearProjectBodyImage } from "../actions";

export function ProjectBodyImage({
  projectId,
  url,
}: {
  projectId: string;
  ...
  function onPick(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    startTransition(async () => {
      const res = await setProjectBodyImage(projectId, fd);
    ...
    <div className="mt-1">
      {url ? (
        <div className="relative inline-block">
          ...
          <img src={url} alt="" className="h-32 w-52 rounded-md border border-[var(--line)] object-cover" />
          <button
            type="button"
            onClick={() => startTransition(async () => { await clearProjectBodyImage(projectId); })}
            disabled={pending}
